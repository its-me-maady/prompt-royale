...
export async function POST(req: Request) {
  try {
    ...
    const customDeps = {
      retrieveEmbeddings: async (q: string) => {
        const embeddings = await embeddingApi.createEmbeddings([q]);
        if (!embeddings || embeddings.length === 0) return [];
        
        const results = await vectorDb.search(embeddings[0], 0.5, 5, filter);
        
        return results.map((row: any) => ({
          id: row.id || row.content,
          content: row.content,
          score: row.similarity || 0.9
        }));
      },
    ...
    const result = await processRagQuery(query, customDeps, { maxContextLength: 4000 });
