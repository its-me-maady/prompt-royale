...
Verify that all expected agent definition files exist in `.agents/agents/`.
...
## Check 3: AGENTS.md Initialization

Check whether `AGENTS.md` has been customized for this project or still contains template placeholders.

```bash
grep -c '\[Your Project Name\]' AGENTS.md
grep -c '\[Brief description' AGENTS.md
grep -c '\[Language, framework' AGENTS.md
```
...
```bash
# Read the tracking-adapter comment from AGENTS.md
grep 'tracking-adapter:' AGENTS.md
grep 'project-number:' AGENTS.md
grep 'project-owner:' AGENTS.md
```
...
- WARN if adapter is set and `gh auth status` succeeds but `project-number` is empty.
  - Suggest: "Run `/kickoff` Phase 5 to create a project board, or set `project-number` in AGENTS.md manually."
