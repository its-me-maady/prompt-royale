---
name: sync-ghcp
description: Sync the Antigravity agent/persona system to GitHub Copilot format.
---
...
### 1. Read the Antigravity Source of Truth
...
Based on the current GHCP docs, build a mapping from Antigravity tool names to GHCP equivalents. As of the last known state, the mapping was roughly:
...
For each `.agents/agents/*.md` file, create or update the corresponding `.github/agents/*.agent.md` file:
...
Create or update `.github/copilot-instructions.md` based on `AGENTS.md`:
...
- Any Antigravity features that have no GHCP equivalent.
