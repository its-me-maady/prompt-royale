...
<!-- agent-notes: { ctx: "resume from previous session handoff", deps: [.agents/handoff.md, AGENTS.md, docs/plans/], state: active, last: "grace@2026-02-12" } -->
Resume from a previous session's handoff.

## Steps

1. **Read the handoff file.** Read `.agents/handoff.md`. If it doesn't exist, tell the user there's no handoff to resume from and ask what they'd like to do.

2. **Read project context.** In parallel, read:
   - `AGENTS.md` (already loaded, but review for any recent changes)
...
3. **Verify state.** Run these checks:
   ...
   - **GitHub Board Pre-Flight (Blocking):** If `AGENTS.md` has a `project-number` and `project-owner` configured (not commented out / not blank), run the full pre-flight:
...
7. **Execute.** Proceed with the "What To Do Next" items from the handoff. Follow all AGENTS.md conventions (TDD, conventional commits, agent-notes, etc.).
