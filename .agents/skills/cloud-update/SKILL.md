---
name: cloud-update
description: Research and update the cloud service landscape for
---
...
After updating, briefly check whether any agent system prompts in `.agents/agents/` (specifically `cloud-architect.md`, `cloud-costguard.md`, `cloud-netdiag.md`) contain hardcoded recommendations that are now wrong. Flag but don't modify agent prompts — those are structural.
