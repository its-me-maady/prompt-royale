...
2. **Dispatch:** Use the `invoke_subagent` tool to spawn the appropriate agents for each sub-task. 
   - CRITICAL: You MUST set the `Workspace` parameter to `"share"` when spawning these agents! This takes advantage of Antigravity's native integration to automatically provision a parallel git worktree for each agent, allowing them to work concurrently without stepping on each other's git state.
